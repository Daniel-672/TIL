...
repeat{
  ...
  print(reple_v)
  reple <- append(reple, unlist(reple_v))
  cat(paste('curr_PageNewNum :', curr_PageNewNum,'\n'))
  curr_PageNewNum <- curr_PageNewNum + 1
  cat('총누적댓글수 :', length(reple), "\n")
  Sys.sleep(1)
  
  if( lastFlag == 'direction next disabled')  {
    cat("종료\n")
    break; 
  }
  ...
  lastFlag <- sapply(pageLink, function(x){x$getElementAttribute('class')})
